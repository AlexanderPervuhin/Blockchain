...
class BlockChainFactory {
    ...
    void extendByOneBlock(BlockChain blockChain) throws ExecutionException, InterruptedException {
        ...
        ArrayList<Transaction> transactionsCache = new ArrayList<>(blockChain.transactionsBuffer);
        blockChain.transactionsBuffer.removeAll(transactionsCache);

        for (int i = 1; i <= threads; i++) {
            BlockMiner miner = new BlockMiner(blockChain);
            miner.setId(i);
            miner.transactions = transactionsCache;
            miners.add(miner);
        }
